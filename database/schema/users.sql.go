...
SELECT user_id, firstname, lastname, email, password, verification_code, is_verified, created_at, updated_at, deleted_at FROM users
...
func (q *Queries) GetUserByEmailAndVerify(ctx context.Context, email string) (*User, error) {
	row := q.db.QueryRowContext(ctx, getUserByEmailAndVerify, email)
	var i User
	err := row.Scan(
		&i.UserID,
		&i.Firstname,
...
func (q *Queries) GetUserByIdAndVerify(ctx context.Context, userID int32) (*User, error) {
	row := q.db.QueryRowContext(ctx, getUserByIdAndVerify, userID)
	var i User
	err := row.Scan(
		&i.UserID,
		&i.Firstname,
